...
> - **What's owed (pen):** the rate/phase/localization of the deviation field `δ_r` — `Σ_r [S_r⟨δ_r,Re w⟩ + Λ_r^unif] = −1/10`, now the whole theorem, via the explicit transport operator `θ̂′(k)=𝔼_v𝔼_t[e(k·β(2⁻ᵛ4ᵗ)/3^r)]`. ~30 honest walk-backs logged across the program (incl. #31, the dense-flat kernel); the deviation candidate `1/(5·21^{k−1})` and the crown ℓ₀-route are both in the kill record.
