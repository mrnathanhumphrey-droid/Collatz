...
> - **The chirp kernel + two-campaign merger (R11) — A/C/D PASS.** `2Λ_r = ε_{r+1}−ε_r` (ε_k = S_k−7/15) **welds the 7/15 value campaign to the corpus's old ε-rate campaign** (ρ≈0.984, period-9.2) — sum and tail of one frozen sequence. The chirp unitary `U(k,ξ)=3^{−r}Σ_z e((kβ(z)−ξz)/3^r)`, `β(z)=dlog₄(1+3z)`, transports `μ̂ → ν̂` exactly and is **shell block-diagonal** (so layer mass = shell mass is a theorem of the bijection), and `Λ_r = ⟨μ̂, K_rμ̂⟩` with `K_r = U*diag(w)U` — an additive quadratic form with the kernel written down. The exact ledger is extended to a **new value Λ₆ = −3.39e−4** (built μ₇ for S₇); Λ signs `−,−,+,+,+,−` show the period-9 oscillation entering. (Honest partial: U is a *sparse* unitary, flat-on-support but not dense-flat — the "maximally-spreading" reading corrected. Angular: Λ_r is carried entirely by near-cancellation between angle-halves.)
> - **What's owed (pen):** the one stationary object in closed form — `γ_∞(τ)` / `C̄_∞(j)` / `Σ_{r≥1}Λ_r = −1/10`, equivalently the decorrelation rate ρ of ⟨profile, weight-phase⟩ — coordinates of the same limit (Theorem S's L→∞ level). ~29 honest walk-backs logged across the program; the deviation candidate `1/(5·21^{k−1})` and the crown ℓ₀-route are both in the kill record.
