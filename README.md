...
> - **What's owed (pen):** *refined* by R18 — the difficulty is **not a norm contraction** (‖δ‖² does not decay; the max coefficient spikes) but a **low-frequency-decay / regularity** statement: summable decay of the fixed-frequency coefficients `A_r(m)=C_{r+1}(m)/3` (the C-tables — full circle to R12-B), with the max/triangle route confirmed dead. Still the same object as R5's qx+1 open step (bulk sum ≈ −9.85e−4). ~32 honest walk-backs logged across the program (Thread-3 adds #31/#34/#35 plus two R18 self-corrections; #32/#33/#36/#37/#39 pre-registered but not incurred); the deviation candidate `1/(5·21^{k−1})` and the crown ℓ₀-route are both in the kill record.
