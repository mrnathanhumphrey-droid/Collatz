...
> - **What's owed (pen):** the uniform tower-contraction estimate on the signed field — equivalently the rate/phase of the bulk sequence `b_r` (sum ≈ −9.85e−4) — now shared with R5's qx+1 open step. ~32 honest walk-backs logged across the program (Thread-3 adds #31/#34/#35; #32/#33/#36/#37 pre-registered but not incurred); the deviation candidate `1/(5·21^{k−1})` and the crown ℓ₀-route are both in the kill record.
