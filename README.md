...
> - **Lobe constant + ψ-existence + transport (R13) — D PASS, C decider OPEN.** The weight is closed-form `Re w = 15/(2D)−½` (mean 0, `‖w‖²=1/15`, the Mersenne `4²−1` = denominator of 7/15). **The chirp β is the renewal itself**: `t′=β(2⁻ᵛ4ᵗ)` and `X_{n+1}=1+3·2⁻ᵛX_n` both reproduce the frozen measure exactly (R13-D). ψ is *not uniform* (an oscillating ~2.5% deficit, depletion concentrated near the trivial character x≈0 — a candidate mixing signal). **The ψ-existence decider is open at reachable depth**: `γ_r(τ_m)` converges cleanly only for m=1; the rest are dominated by a period-9 oscillation with a common r≈6 turnover (m=9 tail-growing), so whether a limiting ψ exists is undecided at r≤7 and needs r≥8 or an analytic damping argument.
> - **What's owed (pen):** ψ-existence (does the r≈6 turnover damp?), then the closed form for `Σ_{r≥1}Λ_r = −1/10` as the transient of the explicit transport operator `θ̂′(k)=𝔼_v𝔼_t[e(k·β(2⁻ᵛ4ᵗ)/3^r)]` (ψ its fixed point). ~30 honest walk-backs logged across the program (incl. #31, the dense-flat kernel); the deviation candidate `1/(5·21^{k−1})` and the crown ℓ₀-route are both in the kill record.
...
| [`results/result_psi_R13.md`](results/result_psi_R13.md) | **R13** — closed-form weight `Re w=15/(2D)−½`; chirp β *is* the renewal (`t′=β(2⁻ᵛ4ᵗ)`, gate); ψ not uniform (depletion near x≈0); ψ-existence decider **open** at r≤7. |
