...
> - **Typical or exceptional (R19) — the decider.** The additive max-spike (R18-D) and the A-side coefficients are **not the same obstruction.** The additive argmax is always a **power of 2 mod 3^r** — pinned to the ⟨2⟩ orbit (the shift generator, the R17 slow mode = 2-adic resonance), finest stratum, angle wandering. The A-side dominant spike is at `m*=3^{r−1}` — a **power of 3**, the coarsest order-3 mode, magnitude ≈0.233 constant. **Orthogonal orbits, opposite strata.** And the fixed low-harmonic coefficients `A_r(m)`, `m∈{1,2,3,4}` — the ones `b_r=⟨δ_r,Re w⟩` actually couples to, since Re w is smooth — are **typical or depleted (O(1), oscillating, not growing)**. So R18-D's max-death is an additive-⟨2⟩ statement that **does not kill the equipartition / regularity route.** (Within strata there *are* isolated spikes, 40× at r=7 — refining R18-B's equipartition to a stratum-average — but they land on the resonance orbits, away from the small m.)
> - **What's owed (pen):** *refined* by R18–R19 — the difficulty is **not a norm contraction** (‖δ‖² does not decay; the max coefficient spikes) but a **low-frequency-decay / regularity** statement: **summable cancellation of the low-harmonic coefficients `A_r(m)=C_{r+1}(m)/3`** (the C-tables — full circle to R12-B), now known to be **uncontaminated by the additive spike** (which lives on the ⟨2⟩ orbit, a different direction). Still the same object as R5's qx+1 open step (bulk sum ≈ −9.85e−4). ~32 honest walk-backs logged across the program (Thread-3 adds #31/#34/#35 plus two R18 self-corrections; #32/#33/#36/#37/#39 pre-registered but not incurred; R19 all-measurement, none incurred); the deviation candidate `1/(5·21^{k−1})` and the crown ℓ₀-route are both in the kill record.
...
| [`results/result_transport_R16.md`](results/result_transport_R16.md) | **R16** — transport step exact (b_r = PASS-gated operator output, δ₁=0 no initial condition); crux = uniform tower contraction = **same as R5 qx+1 open step**; q-oscillation is q=3-critical. |
...
| [`results/result_exceptional_R19.md`](results/result_exceptional_R19.md) | **R19** (the decider) — additive spike **∈⟨2⟩** (power of 2, R17 slow mode); **fixed low-harmonic `A_r(m)` are typical/depleted, not exceptional ⟹ equipartition route survives R18-D**; A-side spike `m*=3^{r−1}∈⟨3⟩` (coarsest, ≈0.233 const) — **orthogonal orbit, two distinct obstructions**. |
