...
> - **Support law + closed loop + lobes (R12) — A/B PASS, F certified.** The chirp support law `U(k,ξ)=0 unless k≡ξ mod 3` is exact (U = two conjugate dense-flat blocks split by the frequency's ± class); every angular moment of the profile is a banked C-table entry (`Σ|θ̂(k)|²e(km/3^r)=C_{r+1}(m)/3`), so the four representations (γ, strata, layers, kernel) **close on each other with no fifth coordinate**. The corpus's exact ε-table is certified byte-identical to `d_k=S_k−7/15` (exact through k=8), extending the exact ledger to **Λ₇** (signs `−,−,+,+,+,−,+`). New observable: the two lobe masses `L_r, M_r` both **stabilize to a common ≈0.0358**, with `Λ_r = L_r−M_r` the tiny oscillating residue — evidence a limiting angular density ψ exists with `⟨ψ, Re w⟩ = 0`.
> - **What's owed (pen):** the one stationary object in closed form — `γ_∞(τ)` / `C̄_∞(j)` / `Σ_{r≥1}Λ_r = −1/10`, equivalently the lobe-difference limit `L_∞−M_∞=0` (with −1/10 the integrated transient) or the Tao-side quadratic form `⟨μ̂, K_rμ̂⟩`. ~29 honest walk-backs logged across the program (incl. #31 above); the deviation candidate `1/(5·21^{k−1})` and the crown ℓ₀-route are both in the kill record.
...
| [`results/result_lobes_R12.md`](results/result_lobes_R12.md) | **R12** — support law `U(k,ξ)=0 unless k≡ξ mod 3`; closed loop (angular moments = C-table entries); ε-table certified = d_k (exact k≤8); Λ₇ exact; lobes L_r,M_r → common ≈0.0358. |
