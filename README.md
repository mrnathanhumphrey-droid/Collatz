...
> - **Endpoint split + bulk correlation (R15) — A gate PASS, B forced.** The corrected retarget `Λ_r = S_r·b_r + Λ_r^unif` is exact (r=2…7), with `Λ_r^unif` the measure-free R10-C trace. The ± reframe is gated: since `N=3^r` is odd, **neither x=0 nor x=½ is a primitive angle** (no self-conjugate fixed point), so `Λ_r^unif` is a primitive-sampling residual, not a literal endpoint atom. Structural finding: **the −1/10 is ~99% the r=1 uniform baseline** (`Λ₁=−2/21`, bulk₁=0); the genuine oscillating deviation-field content is the small bulk sum `Σ_{r≥2} S_r·b_r ≈ −9.85e−4`. (Honest non-fit: the two-band 5/3 sign story does not hold — the band ratio oscillates and never crosses 5/3.)
> - **What's owed (pen):** the rate/phase of the bulk sequence `b_r`, whose sum `Σ_{r≥2} S_r·b_r = −1/10 − ΣΛ_r^unif ≈ −9.85e−4` is now the (small, cleaner) whole theorem, via the explicit transport operator `θ̂′(k)=𝔼_v𝔼_t[e(k·β(2⁻ᵛ4ᵗ)/3^r)]`. ~30 honest walk-backs logged across the program (incl. #31); the deviation candidate `1/(5·21^{k−1})` and the crown ℓ₀-route are both in the kill record.
...
| [`results/result_endpoint_R15.md`](results/result_endpoint_R15.md) | **R15** — endpoint identity `Λ_r=S_r·b_r+Λ_r^unif` exact (anchors byte-exact); no self-conjugate primitive angle (odd lattice); −1/10 is ~99% uniform baseline, bulk sum ≈−9.85e−4. |
