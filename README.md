...
**Status (2026-07-28):** Live state in [`STATE.md`](STATE.md). **Latest — the constant is `S_∞ ≈ 0.475` (exact floor `2·T₂₀ = 0.473177`); 7/15 is dead on value and tail, and the open question is now S_∞'s arithmetic TYPE.** This session mapped the whole neighborhood of that question and found the type **walled in every framework with a name for it**: S is proven **infinite-order Mahler** (= unbounded unipotent depth), the sibling **(q,p)-Hydra family** obeys a clean graded law `S_{k+1}/S_k → q(p−1)/(p+1)` with **Collatz the unique doubly-critical point**, and the **mirror map** `(q,p)=(2,3)` — the only other critical point — has a *different* value (`0.459 ≠ 0.475`) despite the *same* depth: **each critical constant is algebraically alone**. The recurring verdict: the STRUCTURE / associated-graded is symmetric and law-governed; the VALUE is walled. See the [2026-07-28](#latest-2026-07-28) entry. The repo has accreted research threads on top of the original prefix-decomposition / Tao-bridge / qx+1-Cramér work — see "Recent landmarks" below.

## Table of Contents

- [Latest (2026-07-28) — the arithmetic-type neighborhood, walled (Mahler ∞-order, (q,p)-Hydra law, the mirror)](#latest-2026-07-28)
...
<a id="latest-2026-07-28"></a>
> **Latest (2026-07-28) — the arithmetic-type question mapped and walled: S_∞ is infinite-order Mahler, the (q,p)-Hydra family obeys a graded law with Collatz doubly-critical, and the mirror confirms each critical constant is algebraically alone.**
> With the value pinned (`S_∞ ≈ 0.475`, floor `2·T₂₀ = 0.473177`) and 7/15 dead on both value and tail, the live question is S_∞'s *arithmetic type* (rational/algebraic/transcendental). This session mapped its whole neighborhood. The recurring verdict: **the STRUCTURE / associated-graded is symmetric and law-governed; the VALUE is walled and idiosyncratic** — the same split in four independent frameworks. Result docs [`result_MAHLER.md`](results/result_MAHLER.md), [`result_PHYDRA_FAMILY.md`](results/result_PHYDRA_FAMILY.md), [`result_MIRROR.md`](results/result_MIRROR.md), [`result_SCOPING_HEIGHT_BIEXT.md`](results/result_SCOPING_HEIGHT_BIEXT.md). Wilson derives (pen); agents gate (exact where rational, machine-precision).
> - **S is infinite-order Mahler, proven (MAHLER).** No finite-order rational recurrence holds at *any* order: an order-`L` recurrence forces the denominator-bit growth ratio below `λ_L < 2`, but the measured ratio is `3` (the doubly-exponential denominator theorem). So `3 > 2 > λ_L` at every `L`. Infinite Mahler order = the **unbounded unipotent depth** of `M = D(I+N)` (the growing nilpotency index *is* the growing Mahler order). Consequence: finite-order methods (Nishioka decidability, spectral, valuation, lattice) provably cannot reach `S_∞` — the wall stated as a named dichotomy.
> - **The sibling family obeys a unified graded law; Collatz is doubly-critical (PHYDRA_FAMILY).** The genuine siblings vary the valuation prime: the Siegel `(q,p)`-Hydra `x ↦ (qx+1)/p^v`. Across both axes (and mixed cases to `~1e-8`), **`S_{k+1}/S_k → q(p−1)/(p+1)`**. The mass converges iff the ratio is `1` — the **boundary curve `q(p−1) = p+1`** — and **Collatz `(3,2)` is the unique doubly-critical member** (boundary of *both* one-parameter slices at once). This is a genuine functional equation, but for the associated **graded** (the divergence rate / `𝔾_m` / `D` part), *not* the value: at Collatz the rate is marginal (`1`) and the value still lives in the infinite-depth `N` part.
> - **The mirror — the only other critical point — has a different value at the same depth (MIRROR).** The family involution `q = (p+1)/(p−1)` sends Collatz `(3,2)` to the **mirror map** `x ↦ (2x+1)/3^v = (q,p)=(2,3)`; these are the *only two* constructible critical integer points. Computed for the first time: `S_∞(2,3) ≈ 0.459 ≠ S_∞(3,2) ≈ 0.475` (equality decisively refuted, gap `−0.011`), yet the mirror is the **same arithmetic type** (denominators ignite doubly-exponentially via `⟨2⟩`-cofactor primes `41∣2²⁰−1`, `193∣2⁹⁶−1`). So the involution is a symmetry of the criticality *condition* and the *type*, but **not of the value** — **each critical constant is algebraically alone**, the theorem-flavored outcome.
> - **The pairing structure has an abstract home; the height (value) does not (biextension scoping).** `S_∞`'s biextension pairing — axioms 1–3 (bilinear, symmetric, lifting-independent) — is homed in Eskandari's filtered-abelian generality (unipotent case, no geometry or finite-rank needed). But **axiom 4, the real-number height that attaches `0.4749`, is walled**: every number-attaching framework needs a realization our q-difference infinite-order object lacks (Nakayama is geometric, Hardouin is differential). Live **port lead**: Nakayama's asymptotics-under-unipotent-degeneration *retrodict* the measured 3-adic tameness — the first import all session to match a measurement; open gate is whether `⟨C_ρ, K⟩` is his Green-current archimedean height (caveat: our kernel `K` has row-sum `3 ≠ 0`).
...
> - **Frontier update (2026-07-23) — 7/15 is now conditional, and the test is built.** Direct `ν`-computation of `Λ_r` past the ε-data (Probe CROSSING, [`result_crossing.md`](results/result_crossing.md)) plus the exact ε-chain (Probe LOG-PERIODIC, [`result_logperiodic.md`](results/result_logperiodic.md)) show the shell sequence is a **log-periodic bore** whose half-cycles *lengthen* — so every fixed-period estimate was misspecification, and (walk-back #45) a no-turnover extrapolation of the deepest data gives `S_∞≈0.4766`, making **7/15 conditional on an unobserved turnover near r≈27–31**. A **multilevel-splitting estimator is now validated** ([`result_splitting_gate.md`](results/result_splitting_gate.md)) to reach past the `build_nu` wall: γ reproduced to 0.1% (unbiased, rare event `p≈7×10⁻⁸`), and the cumulative-ε turnover test **telescopes to a γ-difference** at SNR≈8–9 — making the turnover an *observable* (deep run pending a bigint accumulator + ESS monitor).
