...
- **L2 (Perron) [TP, ~R8 + Perron–Frobenius].** `λ_1 = 1/3` is a simple, strictly dominant eigenvalue of `L` on the **diagonal** subspace, with the explicit diagonal eigenvector. `C_q ≥ 1` is forced (Cauchy–Schwarz, R8). *(Already essentially in hand.)* **[NUM R32]** For `q ≥ 5`, `λ_1` is simple with a clear gap; for `q = 3` the top two eigenvalues are DISTINCT at finite truncation `L` (0.333336, 0.333236 at L=3) and **merge to 1/3 as L → ∞** — the degeneracy that gives linear growth is a *limit* phenomenon (two eigenvalues colliding), realized as a Jordan block only in the limit, not a finite-L coincidence.
...
- **L3 (THE GAP) [TP, the crux — and NO literature route; see `PHASE3_LITERATURE_GATHER.md`].** On the **off-diagonal** (`a ≠ b`) subspace, `spec-radius(L_off) < 1/3` for `d = ord_q(2) ≥ 3` (i.e. `q ≥ 5`), **uniform in k**.
  - **[NUM R32 — the operator target is now concrete and CLEAN.]** `r_q` is the subdominant eigenvalue of `M` restricted to the **Krylov subspace of the initial vector `v₀`** (the dynamically-relevant subspace). Verified at q=3 (L=3), q=5, q=7 (L=2): the near-`λ₁` "tower" modes (`|μ| ≈ 0.95–1`) have amplitude **exactly 0** — they live *outside* the Krylov subspace and never enter `cross(k)`. So on the relevant subspace `r_q` genuinely IS the dominant subdominant, well-separated (q=5: `r_5 = 0.603`, next amplitude-carrying mode at `0.49` with 60× less weight), gapped `0.40` below `λ_1`. **This is `L_off` made rigorous** — a subspace restriction, not an ad-hoc projection. `L3` = "the subdominant eigenvalue of `M|Krylov(v₀)` is `< λ_1` for `d ≥ 3`, uniformly as `L → ∞`." (Operator VALUE is L-truncation-biased — q=7 L=2 gives 0.475 vs true 0.39 — so accurate `r_q` comes from direct measurement R27; the *gap existence* is what L3 needs and it is structurally robust.)
