...
> ⚠️ **CORRECTION (2026-07-16, Phase-0 gate G0 — see `result_G0_phase0.md`): the H_GAP claim below is WALKED BACK.** "L_k's gain = r_q / σ_max(L_k) = √(r_q/3) < 1/√3" FUSED two different objects. L_k propagates the **/q uniform-lift** deviation, whose gain is **1/√3 for EVERY q (gapless)** — G0 measured γ_5 = γ_7 = 0.577, NOT √(r_q/3) = 0.45/0.36. The gap r_q lives in the **/3 Perron (second-moment)** object `cB_k` (build_M / R42), which R46 measured correctly but mislabeled as L_k's gain. At q=3, /q=/3, so R45 and the q=3 findings here (H_ECHO 2/3-death, FORGET/ONE-STEP/REFINE identities) STAND. The q≥5 "gap from L_k" is the error. r_q is unchanged (still build_M/R42's 0.62, 0.39). See G0 for the full separation.

**Verdicts: H_ECHO (2/3) ✗ REFUTED (injection ≡ 0; propagated fraction = 100%, not 2/3) — STANDS / H_GAP ⚠️ WALKED BACK by G0 (L_k is first-moment/gapless; r_q is second-moment) / Three structural identities at ~1e-16 — STAND.**
