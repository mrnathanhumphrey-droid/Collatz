...
machine precision, all three levels). **Structural reason:** the m=1 channel is a *lag-1* (odd-lag) dlog autocorrelation,
and lag-1 connects opposite dlog-parities; same-parity a stays within one coset of ⟨4⟩⊂⟨2⟩ and cannot span the odd lag,
so it contributes 0. The parity a mod 2 IS the ⟨2⟩/⟨4⟩ = ℤ/2 coset = the `(−1)^a` quadratic-character structure that
BRIDGE2 identified (the even primitive character's ℤ/2). **So `d₁ = ⟨cross-parity interference⟩`, exactly, and
`d₁ > 0` ⟺ that interference is positive.**
...
(FOURCELL, CARRYLEMMA) circled without pinning. **Pen target, now exact:** `d₁ = ⟨even×odd cross-term⟩ > 0`; combined
with the odd-lag/even-lag prediction (odd-lag m = cross-parity, even-lag m = same-parity — testable), this is the
