...
**Headline: ρ₋ = 0 EXACTLY at all 18 grid points — the e=−1 sub-block is genuinely NILPOTENT (nonzero entries, but no cycle), nilpotency index = 2L. This is stronger than P1 (`ρ₋ < |λ₂|`): the off-diagonal sector cannot sustain ANY mode. Claude's "two-step kill" is confirmed as literal nilpotence — at L=1 the block satisfies Msub² = 0 (mass exits in ≤2 steps); at L=2, Msub⁴ = 0 (2 steps per carry level).**
...
- **P3 — q-flat and L-stable. ✅ CONFIRMED (q-flat: spread exactly 0).** Spectral radius is L-stable (0 at every L). **Refinement (observation, not a P-deviation):** the *nilpotency index* grows as **2L** (depth-to-death = 2 per carry level) while ρ₋ stays 0 — so the graph argument's "kill within 2 steps" is per-level, and the once-for-all-q form is justified (q-independent structure).

## What this hands the proof
D1's maximality is now a **confirmation, not an exploration**: the finite reachable-graph argument reduces to *"the `e=−1` reachable carry-subgraph is acyclic"* — provable once for all odd q (P3 q-flatness), with the carry gate `γ ± 2a ≡ 0 mod q` supplying the exit within 2 steps/level (the nilpotency index 2L). **D1 fully closes** on the numerical side; the pen-and-paper acyclicity proof (Nathan) meets a ρ₋ = 0 target, not a `< |λ₂|` inequality.
