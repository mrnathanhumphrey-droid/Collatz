...
| **the spectral gap (the gatekeeper)** | does the gap survive as ε→0 (⟺ C continuous at ½ ⟺ route closes)? | **R25** — drop X_∞, use **C(λ)=lim S_r/ρ^r** (ρ=exact leading eigenvalue); theorem = **boundary value C(½)=7/15**. p₁₄(ε)→7/15 monotone. Measured |λ₂|/ρ: 0.69,0.57 @ε=0.1,0.05. ⚠️its "narrows→1 @small ε" reading **RETRACTED by R26** (artifact-as-trend, #32/#40) | **R25** | result_gap_R25 |
| **the direct eigenvalue (gap resolved)** | is |λ₂|/ρ<1 (gap survives) or →1 (shuts)? | **R26 GAP SURVIVES, ROUTE CLOSES**: Wilson's derived **|λ₂|/ρ=2λ²** (pair-gap operator) CONFIRMED — measured 0.6841 @ε=0.1 vs 0.95·2λ²=**0.6840 (4-digit)**, 0.588 @ε=0.05; clean points **DECREASE** (reverses R25-C). Law → |λ₂|/ρ(½)≈**0.475–0.5 <1** ⟹ **gap survives, C continuous @½, C(½)=7/15 by eigenvalue perturbation; "gap shuts" REFUTED**. ⚠️E3 direct-at-½ BLOCKED (critical period ~20+ > window r≤16; exact finite matrix = R16 growing-spaces, not constructed) — |λ₂|(½)≈½ on 3 converging lines (2λ², subcrit trend, R18-A 0.493/0.503) | **R26** | result_eig_R26 |
| **the recurrence fit (on Λ_r, exact)** | is there an exact recurrence (closed form)? nail |λ₂| & period | **R27 MIXED**: **jackpot DEAD** (4 exact solves disagree ⟹ no finite recurrence, no closed-form ΣΛ); but clean solves give **|λ₂|≈0.5 REAL** ({3,4}=0.503, =R18-A) ⟹ gap-survival **reinforced** (dominant mode NOT a near-circle pair); **≥3 modes** (3-term residual nonzero, 2-mode sign test fails); **period-9 DEMOTED** (appears @9.010 but secondary \|λ\|≈0.19); κ method-dependent, not pinned. Fine structure needs the analytic gap-operator (R27-E) | **R27** | result_recurrence_R27 |
| **the gap-correlation operator (R27-E)** | does Wilson's derived operator close? is the gap bound rigorous? | **R28 A GATE PASS**: **X_r=Σ_d P(d)R_{r−1}(d) EXACT r=2..7** (increments=S_r), **Diagonal Flatness exact**, ⭐**odd-gap conjugate-kill DERIVED (odd-d R = machine zero)** — R7's kill now has a source; even-d = v₃(m) strata. **R27-E CERTIFIED, #43 not incurred**. But bound **NOT rigorous**: **B** twisted<untwisted (κ<1 direction) but ≠ eigenvalue ratio; **C** channel mixing **O(1)** (m=±3 > m=±1) ⟹ `\|λ₂\|/ρ<2λ²` mechanism-level only; **D** deflation-by-0.5 FAILED (ill-conditioned), period still open. Owed: **gap-channel mixing control** | **R28** | result_gapop_R28 |
| **the gap matrix (eigenproblem)** | diagonalize the gap-index operator → \|λ₂\| & period directly? | **R29 MATRIX METHOD FAILS**: **A fails sanity gate** (leading eig=**1.08 ≠ 1**; `3diag(κ)K` has spurious mode from 3K symbol peaking 5/3; physical R grows linearly=defective eig-1 the matrix misses); **B** \|λ₂\| non-convergent in D (→1 not ½) ⟹ A void; **C** κ_2 **drifts** (0.486→0.360, doesn't stabilize) — the obstruction. **BUT D: \|λ₂\|≈½ CONFIRMED independently** (exact Λ ratios 0.4927/0.5028 straddle ½) ⟹ **gap-survival unchanged**, rests on numerical lines not a rigorous matrix. Correct operator ≠ diag(κ)K; stable object = S_r-normed channels. Period open | **R29** | result_gapmatrix_R29 |
| **the digit probability** (target corrected: `Σ_r\|A_r(m)\|<∞`, √-cancellation retired) | recast as conditional-digit equidistribution; is the digit process a clean 3-state chain? | **R30 A GATE PASS**: identity **A_r(m)=γ_{r−1}(3q_r−1)=γ_r−γ_{r−1} EXACT** m=1..4 r=2..7 — **NEW weld collision-γ ↔ character-ledger at m≥1** (m=0 = R10-B/R28 A_r(0)=S_r); certifies `q_r=p_r/p_{r−1}` recasting, **q_r→1/3 ⟺ A_r→0** confirmed. But NOT the clean simplification: **B** per-channel digit rate neither clean ½ nor m-indep (m≥2 3q−1 sign-flips=complex pair; m=1 drifts 0.43→0.9; channel mixing R28-C contaminates); **C** excess **ALTERNATES** d=1↔d=2 by r-parity (\|λ₂\| pair in digit space, not static skew); **D** q depends on **which** digits matched (mod-27 spread 0.06–0.12) ⟹ NOT scalar 3-state chain, BUT spread **decays** (0.06→0.007) ⟹ asymptotically depth-Markov. Owed: control the homogenization rate (= the same \|λ₂\|≈½ gap, better-shaped) | **R30** | result_digitprob_R30 |
| **M-reality** (the S=−2M chain: was it proved?) | is `Im M_n(η)=0` proved or empirical? the whole c=7/45 corpus (R74–R77) hangs on Thm 76.3 `S_{n+1}=−2·M(1+3^n)` | **R80 PROMOTED empirical→rigorous.** Original 76.3 closed M-reality by "numerics + class-symmetry hand-wave." **Lemma 76.0 (unconditional, 2 lines): A={3∤ξ} is −1-closed + fixed-point-free; π real ⟹ μ̂(−ξ)=μ̂(ξ)\*; reindex ξ→−ξ ⟹ M(η)=M(η)\* REAL. Needs only π real, NOT R66 class-symmetry.** ⭐**RETRO DE-RISK: value never depended on it — conservation+Hermitian give `S=−2·Re M(1+3^n)` unconditionally.** Gate: max\|Im M\|≤2.3e−17 over all η, k=7 | **R80** (`gate_M_reality_760.py`) | result_76_conservation_law §1a |
| **the subordination warp** (W1/W3): is \|λ₂\|=½ the affine dressing applied to a solvable core? | split R28 op = `A_*=diag(e(ξ/3^{r+1}))` ∘ core `Σ_v2^{−v}(halving)^v`; is A_* a deformation (½ real) or conjugation (½ artifact)? | **W3 CONFIRMED, W1 REFUTED.** **W1**: unshifted core has **NO gap** (\|λ₂\|→1; analytic 1/(2χ(2)−1), generator char→1), **not** 1/3 — `f(½)=1/3` conflates Bernoulli-convolution (1st moment) with permutation resolvent. **W3 GATE**: shift `A_*` **creates** a stable gap (core→1 ⟹ shifted \|λ₂\|=0.970) ⟹ **genuine deformation, NOT conjugation ⟹ ½ not an artifact, four \|λ₂\|≈½ lines SAFE, shift load-bearing.** But 0.970≠½ (=envelope/period-9); **½ is SECOND-moment**, absent from 1st-moment op. Owed: W2 (76.2 −2 pairing) on the **second-moment** operator | **W1/W3** (`probe_warp_W13.py`) | result_warp_W13 |
| **the tailor** (W2): does 76.2 pairing on the 2nd moment give ½? | build the finite lift-triple operator + read the exact rate; the rigorous finite pieces may carry ½ without the non-existent ∞-operator | **W2 does NOT derive ½ — EXPOSES it as a TRANSIENT.** **A** η=1 triple = `S_n·(1,−½,−½)` EXACT (Thm 76.3), **rank-1 ⟹ degenerate**, no independent ½. **B** the `−2` is exact but a within-level magnitude, not a rate factor. **C/E** exact ladder: ½ holds **only r≤5**, breaks r=6–7 (`\|ε\|·2^k` jumps 4.7× → 0.15, 0.19; `\|Λ\|·2^r` grows; ratios sign-flip) = the corpus's **May refutation of rate-½**, asymptotic **ρ≈0.984**. **Reconcile:** ½ = subcritical/fast mode (R26 stands); at criticality the slower **period-9 ~0.984** (critical-only) dominates ⟹ **true \|λ₂\| = 0.984, not ½.** RETRACTED: ½ as the asymptotic rate | **W2/R81** (`probe_W2_tailor.py`) | result_W2_tailor |
| **the period-9 frontier** (F1): what *is* the dominant 0.984 mode? | pin its structure — integer-vs-irrational period, where it lives, the lattice check | **rate 0.984 confirmed to k=12** (`\|ε\|·2^k → 9.3`, `\|ε\|/.984^k` bounded, `Λ_r` flat r≥7). **Period is NON-integer** — the exact 11-term `Λ` sign seq `−−+++−+++++` excludes every integer 2–9. Mode lives in the **intermediate-j lift tower** (oscillating), not the rank-1 η=1 part; not a clean finite eig. **`√0.97008=0.98493` unearned numerology.** **Lattice: archimedean step NON-lattice** (`log3/log2` irrational — Wilson correct, Feller ⟹ no archimedean oscillation); the oscillation is **p-adic-lattice-FORCED** (Lapidus–Hùng: p-adic strings always lattice) — a signature of the correct category. Rate `<1` ⟹ theorem unaffected; the dominant mode is now *identified* | **F1/R82** (`probe_F1_period9.py`) | result_F1_period9 |
...
decay, here 4^{−m} localizes 94% onto 2 values = structure R5 lacks), plus the **R30 target-correction** (Wilson's
own √-cancellation requirement `max\|μ̂\|≲3^{−r/2}` from R18-D **RETIRED** — that was the triangle-inequality route,
aimed 3 orders too high; the theorem needs only `Σ_r\|A_r(m)\|<∞`, i.e. `O(r^{−1−δ})`, decay observed geometric),
plus two self-corrections in R18 —
