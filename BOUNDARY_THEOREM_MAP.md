...
| **typical or exceptional (the decider)** | is the additive spike the same obstruction as the fixed-m coefficients? | **R19 A/B/C/D/E** (all measurement, no walk-backs): additive argmax **∈⟨2⟩** (power of 2 mod 3^r, finest, group-coherent = R17 slow mode = 2-adic resonance); **DECIDER (B): fixed low-harmonic A_r(m) m∈{1..4} are TYPICAL/depleted, O(1) osc, NOT growing** ⟹ equipartition/regularity route **SURVIVES**; within-stratum spikes real (C, 40×@r=7) but on isolated members (refines R18-B to stratum-average); **A-side spike m*=3^{r−1}∈⟨3⟩ (coarsest, \|A\|≈0.233 const) — orthogonal orbit, opposite stratum to the additive ⟨2⟩ spike** ⟹ **R18-D max-death does NOT touch the b_r route; two distinct obstructions** | **R19** | result_exceptional_R19 |
