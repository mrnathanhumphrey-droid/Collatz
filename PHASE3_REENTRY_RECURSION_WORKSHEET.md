...
3. **[DONE, R38] — the digit corrections TELESCOPE.** σ₂ enters `W₃` with coefficient `+y₁·j₁` — the SAME σ₁ carried into `W₂`. Mechanism: `2^{j₁d} = 1 + q·j₁·τ + q²·C(j₁,2)·τ² + … (τ := (2^d−1)/q)`, so the q-adic digits of the linear term are `j₁·(digits of τ)` — the measured per-digit coefficient. **⇒ ω's dependence on the LTE tower is a closed generating function: the digits `(s, σ₁, σ₂, …)` are NOT independent knobs, they are the base-q representation of the single constant `τ`. Write ω's digit-part as `y₁·j₁·τ` (linear) + `C(j₁,2)τ²` (…). No infinite tower of corrections — the k=2 chain is a truncation of a closed form in `τ`.** `τ` enters benignly (unit iff s=1 regime; `τ≡0 mod q` = s≥2 = R35 onset-shift; `τ=s` alone at q=5,7 which gap). Boundary still `d=2`. [R38]

---

## 9. One-line summary

`c_k = 3^k g(k)` obeys a re-entry recursion `G(k) = 3·Σ_φ κ_φ·ω_φ(d,τ)·G(k−1|φ)`; the **rate factor `κ` is q-uniform** (R36) so **both boundaries live in the phase/weight factor `ω`** — `d=2` saturates its phase-count (FP1, `r_3=1`), `s_R13=0` index-shifts it (FP2, gap survives). `ω` is a character sum (R13), and its dependence on the LTE tower is a **closed generating function in the single constant `τ = (2^d−1)/q`** (R37+R38: digits enter linearly with common coefficient `y₁·j₁` = the q-adic expansion of `q·j₁·τ`; no infinite tower). L3 = "this character sum has spectral radius `< 1/3` for `d ≥ 3`," boundary-anchored in Konyagin/Siegel, positive side novel — now over a clean object (one constant `τ` per prime, not an unbounded digit tower).
