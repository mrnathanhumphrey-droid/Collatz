"""
Experiment 35 — full bridge consolidation for Nicolo package.

Confirms s_mean ~ alpha_det at K_h = 10.4282 with slope = 1 and offset
matching Tao (5.15) leading term across:
  - 4 modular resolutions: k = 8, 10, 12, 14
  - 5 observables: sigma (full descent to 1) and first-passage to
    {N^(2/3), sqrt(N)*log(N), sqrt(N), sqrt(N)/log(N)}
  - 2 data scales: N = 2^25 and N = 2^27

Each (k, observable, scale) cell reports slope and offset deviation from
Tao (5.15) leading term K_h * delta_log. If slope = 1.000 +/- 0.005 and
offset matches K_h * delta_log within 2 steps (raw mean) or 1 step
(trim-1%), the per-class match holds.

Usage:
    python 35_alpha_det_full_bridge.py
"""
import gc
import sys
import time
...
def process_N(N_val):
    ...
    print()
    print("#" * 100)
    print(f"# N = {N_val:,} = 2^{int(round(np.log2(N_val)))}")
    print("#" * 100)
    ...
    for k in [8, 10, 12, 14]:
        ...
        if per_class_min < 100:
            print(f"  k={k}: per-class size {per_class_min} too small, skipping")
            continue
        print()
        print(f"  --- k = {k} (mod {M}, {K_classes} odd classes, per-class ~{per_class_min:,}) ---")
        ...
        print(f"    groupby in {time.perf_counter()-t_k:.1f}s")
        observables = [
            ("sigma",        "sigma"),
            ("s @ N^(2/3)", "s0"),
            ("s @ sqrt*log","s1"),
...
def main():
    for N_val in [33554432, 134217728]:  # 2^25, 2^27
        process_N(N_val)
