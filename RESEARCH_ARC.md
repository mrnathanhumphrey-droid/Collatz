...
**Audit date:** 2026-07-17 (full audit); **frontier refreshed 2026-07-26** (Thread-3 arc P6D–P6G — the cascade constant reduced to one sub-measure; earlier refresh 2026-07-22 for R18–R30; see the 2026-07 row and §Frontier note). Purpose: a single top-level map of the *entire* research program — every result (proven / open / killed / superseded), the chronology, the mathematical fields, the canonical spine, and the repo mechanics. Indexed three ways: **by finding · by month · by field of math**. Built from a full read-only audit of all 1,749 tracked files.

> **Frontier note (2026-07-26) — Thread-3, the 7/15 constant, arc P6D–P6G.** The marginal loss constant question `7/15 ⟺ Σ_{i≥2}Λ_i = −1/210` is now a machine-exact chain reducing the whole cascade to a **single sub-measure `ν_e` (the even-branch Syracuse sub-measure), two lags per level**. Closed form (P6G), gated to 1e-16 through i=6: `Λ_i = 3ⁱ[4R_e⁽ⁱ⁾(2) − R_e⁽ⁱ⁾(0)] − 3ⁱ⁻¹[4R_e⁽ⁱ⁻¹⁾(2) − R_e⁽ⁱ⁻¹⁾(0)]`, `R_e = autocorr(ν_e)`, `Λ₁=−2/21` exact, **no free maps, no fitted constant, no boundary term**. Chain: `ν_e` → [collapse `ν_o=½(×2⁻¹)ν_e+½(m₁)ν`, exact, P6D] → `ν` → [autocorr] → [deconvolve `ν̂=D̃ρ̂`, `|D̃|²=1/(5−4cosθ)∈[1/9,1]`, P6F] → `C_ρ` → [three-point kernel `K̂=4cos2θ−1`, P6G] → `Λ_i`. Positivity `R_e(2) > ¼R_e(0)` = Chebyshev/covariance family (the `m=0` proof's family; first time `m=1` is in-family). Open (pen): factor-4 normalization reconcile + asymptotics of `3ⁱ[4R_e⁽ⁱ⁾(2)−R_e⁽ⁱ⁾(0)]` → whether the tail sums to exactly `−1/210` (`Σ_{2..6}=−0.00535` partial). Docs `results/result_P6{D,E,F,G}.md`; commits `05e8cdd`→`6b057cb`.
...
**Frontier note (2026-07-22).** The live frontier is the **Thread-3 `S_∞ = 7/15` arc** (probes R18–R30, R80–R81 + W1/W2/W3, banked in `STATE.md` + `BOUNDARY_THEOREM_MAP.md` + `results/`). The reformulation program is complete and provably so — every coordinate (γ-collision, strata, character layers, gap operator, ratio density, digit probability) closes on one object. Two late results reshaped the crux: **R80** promoted the `S=−2M` chain's M-reality from empirical to a rigorous two-line lemma (Lemma 76.0), and **R81/W2** showed the long-chased `|λ₂|=½` is a **subcritical/transient** mode — the *asymptotic* rate of `S_n→7/15` is the slower **period-9 ~0.984** (critical-only), reconciling Thread-3 with the corpus's May refutation of rate-½. What remains is a single analytic step: **3-adic second-moment Fourier decay / conditional-digit equidistribution at geometric rate** (`Σ_r|A_r(m)|<∞`; summability needs only rate `<1`). The four-hunt literature map (**novel / robust / derivable**, top lead = Bourgain–Gamburd L²-flattening in ℤ/3ⁿ) is in `THREAD3_LITERATURE.md`. See `STATE.md` for the per-probe log.
